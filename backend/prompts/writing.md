...
**重要：请严格按照以下格式输出，这对于程序正确解析至关重要！**

请直接输出一个标准的 JSON 对象，不要包含 markdown 代码块标记（```json ... ```），也不要包含任何额外的解释文字。
...
```json
{
  ...
  "full_content": "章节完整内容（纯文本，请确保字数充足，建议 4500 字以上）"
}
```

**关键注意事项：**

1. **`full_content` 字段内容要求**：
   - 必须是纯文本字符串，不要包含JSON结构
   - 使用 `\n` 表示换行（两个字符：反斜杠+n）
   - **必须正确转义**特殊字符：
     * 双引号写成 `\"`
     * 反斜杠写成 `\\`
...
3. **内容完整性**：
   - 确保章节内容完整，如果字数较多，要保证全部输出完毕
   - 不要因为长度限制而截断内容，宁可分段描述也要保持结构完整
